...
`);return import_ts_dedent23.default`
    ...
    To learn more about this change, see: ${import_chalk21.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
  `},async run({dryRun,mainConfigPath,result:{nextStoriesEntries}}){logger19.info(import_ts_dedent23.default`✅ Setting 'stories' config:
      ${JSON.stringify(nextStoriesEntries,null,2)}`),dryRun||await updateMainConfig({mainConfigPath,dryRun},async main=>{main.setFieldValue(["stories"],nextStoriesEntries)})}};var allFixes=[nodeJsRequirement,cra5,webpack5,angular12,vue3,eslintPlugin,builderVite,sbBinary,sbScripts,newFrameworks,removedGlobalClientAPIs,mdx1to2,mdxgfm,bareMdxStoriesGlob,autodocsTrue,addReact,missingBabelRc],initFixes=[missingBabelRc,eslintPlugin];var import_os=require("os"),ansiRegex=({onlyFirst=!1}={})=>{let pattern=["[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]+)*|[a-zA-Z\\d]+(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]*)*)?\\u0007)","(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PR-TZcf-ntqry=><~]))"].join("|");return new RegExp(pattern,onlyFirst?void 0:"g")},cleanLog=str=>str.replace(ansiRegex(),"").replace(/╮│/g,`\u256E
\u2502`).replace(/││/g,`\u2502
\u2502`).replace(/│╰/g,`\u2502
\u2570`).replace(/⚠️ {2}failed to check/g,`${import_os.EOL}\u26A0\uFE0F  failed to check`);var logger20=console,LOG_FILE_NAME="migration-storybook.log",LOG_FILE_PATH=(0,import_path12.join)(process.cwd(),LOG_FILE_NAME),TEMP_LOG_FILE_PATH="",originalStdOutWrite=process.stdout.write.bind(process.stdout),originalStdErrWrite=process.stderr.write.bind(process.stdout),augmentLogsToFile=()=>{TEMP_LOG_FILE_PATH=import_tempy.default.file({name:LOG_FILE_NAME});let logStream=(0,import_fs_extra11.createWriteStream)(TEMP_LOG_FILE_PATH);process.stdout.write=d=>(originalStdOutWrite(d),logStream.write(cleanLog(d))),process.stderr.write=d=>logStream.write(cleanLog(d))},cleanup=()=>{process.stdout.write=originalStdOutWrite,process.stderr.write=originalStdErrWrite};var logAvailableMigrations=()=>{let availableFixes=allFixes.map(f=>import_chalk22.default.yellow(f.id)).join(", ");logger20.info(`
The following migrations are available: ${availableFixes}`)},automigrate=async({fixId,fixes:inputFixes,dryRun,yes,useNpm,packageManager:pkgMgr,list,configDir:userSpecifiedConfigDir,renderer:rendererPackage,skipInstall}={})=>{var _a;if(list)return logAvailableMigrations(),null;useNpm&&(useNpmWarning(),pkgMgr="npm");let selectedFixes=inputFixes||allFixes,fixes=fixId?selectedFixes.filter(f=>f.id===fixId):selectedFixes;if(fixId&&fixes.length===0)return logger20.info(`\u{1F4ED} No migrations found for ${import_chalk22.default.magenta(fixId)}.`),logAvailableMigrations(),null;augmentLogsToFile();let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),{configDir:inferredConfigDir,mainConfig:mainConfigPath,version:storybookVersion}=(0,import_core_common6.getStorybookInfo)(packageManager.retrievePackageJson(),userSpecifiedConfigDir);if(!(storybookVersion&&((_a=import_semver20.default.coerce(storybookVersion))==null?void 0:_a.version)))return logger20.info(import_ts_dedent24.default`
      ...
      `),{preCheckFailure:"undetected_sb_version"};let configDir=userSpecifiedConfigDir||inferredConfigDir||".storybook";try{await(0,import_core_common6.loadMainConfig)({configDir})}catch(err){return err.message.includes("No configuration files have been found")?(logger20.info(import_ts_dedent24.default`[Storybook automigrate] Could not find or evaluate your Storybook main.js config directory at ${import_chalk22.default.blue(configDir)} so the automigrations will be skipped. You might be running this command in a monorepo or a non-standard project structure. If that is the case, please rerun this command by specifying the path to your Storybook config directory via the --config-dir option.`),{preCheckFailure:"mainjs_not_found"}):(logger20.info(import_ts_dedent24.default`[Storybook automigrate] ❌ Failed trying to evaluate ${import_chalk22.default.blue(mainConfigPath)} with the following error: ${err.message}`),logger20.info("Please fix the error and try again."),{preCheckFailure:"mainjs_evaluation_error"})}logger20.info("\u{1F50E} checking possible migrations..");let fixResults={},fixSummary={succeeded:[],failed:{},manual:[],skipped:[]};for(let i=0;i<fixes.length;i+=1){let f=fixes[i],result;try{result=await f.check({packageManager,configDir,rendererPackage})}catch(error){logger20.info(`\u26A0\uFE0F  failed to check fix ${import_chalk22.default.bold(f.id)}`),logger20.error(`
${error.stack}`),fixSummary.failed[f.id]=error.message,fixResults[f.id]="check_failed"}if(result){logger20.info(`
\u{1F50E} found a '${import_chalk22.default.cyan(f.id)}' migration:`);let message=f.prompt(result);logger20.info((0,import_boxen.default)(message,{borderStyle:"round",padding:1,borderColor:"#F1618C",title:f.promptOnly?"Manual migration detected":"Automigration detected"}));let runAnswer;try{if(dryRun)runAnswer={fix:!1};else if(yes)runAnswer={fix:!0},f.promptOnly&&(fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id));else if(f.promptOnly){fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id),logger20.info();let{shouldContinue}=await(0,import_prompts4.default)({type:"toggle",name:"shouldContinue",message:"Select continue once you have made the required changes, or quit to exit the migration process",initial:!0,active:"continue",inactive:"quit"},{onCancel:()=>{throw new Error}});if(!shouldContinue){fixResults[f.id]="manual_skipped";break}}else runAnswer=await(0,import_prompts4.default)({type:"confirm",name:"fix",message:`Do you want to run the '${import_chalk22.default.cyan(f.id)}' migration on your project?`,initial:!0},{onCancel:()=>{throw new Error}})}catch{break}if(!f.promptOnly)if(runAnswer.fix)try{await f.run({result,packageManager,dryRun,mainConfigPath,skipInstall}),logger20.info(`\u2705 ran ${import_chalk22.default.cyan(f.id)} migration`),fixResults[f.id]="succeeded",fixSummary.succeeded.push(f.id)}catch(error){fixResults[f.id]="failed",fixSummary.failed[f.id]=error.message,logger20.info(`\u274C error when running ${import_chalk22.default.cyan(f.id)} migration`),logger20.info(error),logger20.info()}else fixResults[f.id]="skipped",fixSummary.skipped.push(f.id)}else fixResults[f.id]=fixResults[f.id]||"unnecessary"}return Object.values(fixResults).some(r=>r==="failed"||r==="check_failed")?await(0,import_fs_extra11.move)(TEMP_LOG_FILE_PATH,(0,import_path12.join)(process.cwd(),LOG_FILE_NAME),{overwrite:!0}):await(0,import_fs_extra11.remove)(TEMP_LOG_FILE_PATH),logger20.info(),logger20.info(getMigrationSummary(fixResults,fixSummary,LOG_FILE_PATH)),logger20.info(),cleanup(),fixResults};function getMigrationSummary(fixResults,fixSummary,logFile){let hasNoFixes=Object.values(fixResults).every(r=>r==="unnecessary"),hasFailures=Object.values(fixResults).some(r=>r==="failed"||r==="check_failed"),title=hasNoFixes?"No migrations were applicable to your project":hasFailures?"Migration check ran with failures":"Migration check ran successfully",successfulFixesMessage=fixSummary.succeeded.length>0?`
      ${import_chalk22.default.bold("Successful migrations:")}

 ${fixSummary.succeeded.map(m=>import_chalk22.default.green(m)).join(", ")}
    `:"",failedFixesMessage=Object.keys(fixSummary.failed).length>0?`
    ${import_chalk22.default.bold("Failed migrations:")}
 ${Object.entries(fixSummary.failed).reduce((acc,[id,error])=>`${acc}
${import_chalk22.default.redBright(id)}:
${error}
`,"")}
    
You can find the full logs in ${import_chalk22.default.cyan(logFile)}
`:"",manualFixesMessage=fixSummary.manual.length>0?`
      ${import_chalk22.default.bold("Manual migrations:")}

 ${fixSummary.manual.map(m=>fixResults[m]==="manual_succeeded"?import_chalk22.default.green(m):import_chalk22.default.blue(m)).join(", ")}
    `:"",skippedFixesMessage=fixSummary.skipped.length>0?`
      ${import_chalk22.default.bold("Skipped migrations:")}

 ${fixSummary.skipped.map(m=>import_chalk22.default.cyan(m)).join(", ")}
...
`}If you'd like to run the migrations again, you can do so by running '${import_chalk22.default.cyan("npx storybook@next automigrate")}'
    
    The automigrations try to migrate common patterns in your project, but might not contain everything needed to migrate to the latest version of Storybook.
    
    Please check the changelog and migration guide for manual migrations and more information: ${import_chalk22.default.yellow("https://storybook.js.org/migration-guides/7.0")}
    And reach out on Discord if you need help: ${import_chalk22.default.yellow("https://discord.gg/storybook")}
  `;return(0,import_boxen.default)(summaryMessage,{borderStyle:"round",padding:1,title,borderColor:hasFailures?"red":"green"})}var logger21=console,installStorybook=(projectType,packageManager,options)=>{let npmOptions={installAsDevDependencies:!0,skipInstall:options.skipInstall},packageJson;try{packageJson=packageManager.readPackageJson()}catch{}let language=detectLanguage(packageJson),pnp=detectPnp(),generatorOptions={language,builder:options.builder||detectBuilder(packageManager,projectType),linkable:!!options.linkable,pnp:pnp||options.usePnp};return(async()=>{switch(projectType){case"REACT_SCRIPTS":return REACT_SCRIPTS_default(packageManager,npmOptions,generatorOptions).then(commandLog('Adding Storybook support to your "Create React App" based project'));case"REACT":return REACT_default(packageManager,npmOptions,generatorOptions).then(commandLog(`Adding Storybook support to your "React" app
