import { getSSHKeyFingerprint } from './ssh-fingerprint';
...
describe('Generating SSH key fingerprints', () => {
  it('should correctly fingerprint an ssh-rsa key', () => {
    expect(getSSHKeyFingerprint(rsa)).toMatch(rsaFingerprint);
  });

  it('should correctly fingerprint an ssh-ed25519 key', () => {
    expect(getSSHKeyFingerprint(ed25519)).toMatch(edFingerprint);
  });

  it('should return a message in the event of an error', () => {
    expect(getSSHKeyFingerprint(null as any)).toMatch(
